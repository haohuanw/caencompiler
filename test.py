...
from caencompiler.test.modeTest import uploadTest, GitTest, InitTest
...
def main():
    ...
    # uploadTest("all", "proj1", "proj1", ["io.cpp", "io.h", "proj1.cpp"], \
            # config["PROJECT_LOCAL_PATH"], config["PROJECT_REMOTE_PATH"]+"uploads/",\
            # config["host"], config["user"], config["passwords"])
    # GitTest("all", "proj1", "proj1", \
            # config["PROJECT_LOCAL_PATH"], config["PROJECT_REMOTE_PATH"]+"git/",\
            # config["host"], config["user"], config["passwords"])
    InitTest("https://github.com/haohuanw/caencompiler.git", config["PROJECT_REMOTE_PATH"], \
            config["host"], config["user"], config["passwords"])
